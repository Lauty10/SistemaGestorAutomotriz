...
void Auto::setNombreAuto(char nombreA[20]){
strcpy(this->nombreAuto,nombreA);
 }

void Auto::setMarcaAuto(char marcaA[20]){
strcpy(this->marcaAuto,marcaA);
 }
void Auto::setPrecioAuto(float precioA){
this->precioAuto=precioA;
 }
void Auto::setEstado(bool estadoA){
this->estado=estadoA;
}
int Auto::getId(){
return idDelAuto;
 }
const char* Auto::getNombreAuto(){
return nombreAuto;
 }
const char* Auto::getMarcaAuto(){
return marcaAuto;
 }
float Auto::getPrecioAuto(){
return precioAuto;
 }
bool Auto::getEstado(){
return estado;
...
//MODIFICAR DATOS
void Auto::modificarAutos(int id) {
    bool control_menu_vehiculo = true;
    int y = 0;
